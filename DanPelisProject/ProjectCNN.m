...
srcFiles = dir('trainingData\16QAM');

for i = 3 : length(srcFiles)
filename = strcat('trainingData\16QAM\',srcFiles(i).name);
im = imread(filename);
k=imresize(im,[126,126]);
newfilename=strcat('trainingData\16QAM\',srcFiles(i).name);
imwrite(k,newfilename,'png');
end

srcFiles = dir('trainingData\32QAM');

for i = 3 : length(srcFiles)
filename = strcat('trainingData\32QAM\',srcFiles(i).name);
im = imread(filename);
k=imresize(im,[126,126]);
newfilename=strcat('trainingData\32QAM\',srcFiles(i).name);
imwrite(k,newfilename,'png');
end

srcFiles = dir('trainingData\64QAM');

for i = 3 : length(srcFiles)
filename = strcat('trainingData\64QAM\',srcFiles(i).name);
im = imread(filename);
k=imresize(im,[126,126]);
newfilename=strcat('trainingData\64QAM\',srcFiles(i).name);
imwrite(k,newfilename,'png');
end
...
filename = strcat('testData\64QAM\64QAM_187.jpg');
im = imread(filename);
k=imresize(im,[126,126]);
newfilename=strcat('testData\64QAM\64QAM_187.jpg');
imwrite(k,newfilename,'png');
